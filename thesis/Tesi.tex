...
\\
\\
Naturalmente, le considerazioni fatte per queste tre tecniche valgono per qualsiasi idioma; tipicamente, le librerie che le implementano presentano delle interfacce per specificare su quale lingua si intende lavorare in maniera da ricorrere a opportuni dizionari.
...
In questa fase ogni documento $i$ è formato da $n_i$ parole ed ogni parola è rappresentata da un vettore di $r$ feature. L'aggregazione interviene per comprimere ciascuno di questi vettori in un valore che sia rappresentativo della corrispondente parola, ottenendo così un vettore per l'intero documento.
\\
...
\\
Alla luce di tutto ciò, questa tesi propone di modellare il problema della rilevazione delle fake news come un problema fuzzy, assumendo che le notizie siano associate a insiemi di questo tipo.
\\
...
\\
Nel Paragrafo \ref{sistema}, invece, si mostra l'utilizzo che è stato fatto dell'algoritmo sopracitato all'interno del sistema, delineando le fasi che si susseguono nel corso degli esperimenti.
...
\\
L'algoritmo viene configurato con ogni combinazione della grid search, generando per ciascuna un modello che viene addestrato col training set, un insieme di dati etichettati necessari per formare la memoria del modello predittivo. Successivamente, il modello osserva i dati del validation set, nuovi dati del campione senza etichetta, e produce delle predizioni. Tali predizioni vengono, dunque, comparate alle etichette originali per misurare la capacità predittiva del modello; questo viene fatto tramite delle apposite metriche e, sulla base di queste, si sceglie il modello effettivamente più efficace.
\\
Il test set, invece, funga da prova finale per valutare la capacità di generalizzazione del modello su dati mai visti.
...
La scelta dei modelli si basa su uno o più criteri di valutazione e, tipicamente, avviene tramite delle metriche di bontà o attraverso delle misure di errore del predittore generato.
\\
...
Nel contesto della classificazione è possibile calcolare delle misure quantitative per valutare la bontà di un predittore e quelle di \textit{precision}, \textit{recall} e \textit{F1} sono basate sulle quattro classi viste nel paragrafo precedente.
...
Il concetto di accuratezza di un modello non è sempre applicabile, spacialmente se si considera un tipo di predizione diverso dalla classificazione che produce valori continui. In questi casi si ricorre a delle misure del concetto di \textit{errore}, ossia la quantità che descrive quanto il valore della predizione sia deviato rispetto alla ground truth.
In questa tesi sono state utilizzate le metriche di \textit{mean squared error} e di \textit{root mean squared error}.
\\
Si fissi il dataset $S$ formato da tutte le coppie $(\mathbf{x_i},y_i) \; \forall\,i=1, ..., m$, dove $\mathbf{x_i}$ indica il vettore delle feature, $y_i$ l'etichetta e $i$ l'indice di scorrimento delle osservazioni:
...
\begin{equation}
    \sqrt{\frac{1}{m}\sum\limits_{j=1}^m (\hat{y_j} - y_j)^2}
\end{equation}
\\
In questa tesi sono state considerate tutte le misure viste sopra, poiché è stato considerato sia il caso di induzione della funzione di appartenenza che ha prodotto valori compresi tra 0 e 1, sia un classificatore binario che applica una funzione di soglia a tali valori per ottenere una risposta binaria.
...
La fase di model selection si occupa di individuare il modello ideale tenendo presente le considerazioni relative alla valutazione e alla bontà di generalizzazione dei modelli fatte nei Paragrafi \ref{evaluation} e \ref{generalization}.
\\
...
I modelli selezionati vengono, dunque, serializzati per permettere il loro riutilizzo in un secondo momento; più precisamente, questo avviene nella terza fase di visualizzazione dei dati in cui si vuole valutare il loro comportamento.
\\
...
\\
Il criterio con cui viene valutata la correttezza delle predizioni consiste nell'utilizzare le misure quantitative di errore descritte nei Paragrafi \ref{erroreclass} e \ref{erroreregr}. Si sottolinea, inoltre, che in questa fase vengono effettuati dei confronti con una determinata \textit{baseline}, ovvero uno o più stimatori di riferimento le cui predizioni vengono valutate sugli stessi dati trattati da $\mu$-learn, per poter fornire un ulteriore metro di paragone. Come mostrato in Figura \ref{datavisualization}, l'analisi può interagire nuovamente col dataset di partenza per poter riosservare manualmente quelle osservazioni che causano incertezza nelle predizioni. Complessivamente, il terzo nodo è importante per poter estrarre maggior conoscenza dall'ambito preso in considerazione. Tutte le conclusioni tratte dall'analisi descrittiva, infatti, vengono riassunte in un report e sulla base di esso, si può eventualmente decidere se ricominciare gli esperimenti.
...
\label{Capitolo 3}
\onehalfspacing
Il Capitolo \ref{Capitolo 3} tratta l'implementazione della soluzione proposta nel capitolo precedente.
\\
...
\\
Ogni operazione segue il pattern di implementazione della libreria \textit{scikit-learn}, che prevede di separare le procedure di \textit{fit} e \textit{transform}, che definiscono rispettivamente la parte di apprendimento dei dati, quando richiesta, e quella di effettiva trasformazione dell'input in un output processato.
\\
...
Inoltre, l'ordine con cui esse vengono applicate è determinante per la rielaborazione del testo: è più semplice rimuovere il rumore analizzando i token piuttosto che le stringhe di intere frasi, ecco perchè la tokenizzazione avviene tipicamente all'inizio della pipeline. Un altro esempio è quello dell'aggregazione che, come visto nei Paragrafi \ref{w2v} e \ref{d2v}, è necessaria solo in caso di utilizzo di Word2Vec.
\\
\begin{figure}
    \centering
...
Nel processo di apprendimento di un task predittivo vengono generati numerosi modelli; per questa ragione diventa cruciale utilizzare un criterio di selezione di tali modelli per ricavare i migliori risultati possibili.
...
\\
Per completezza, nell'Algoritmo \ref{kfold} vengono delineati i passi chiave della procedura: si fa un partizionamento in $k$ fold, organizzandole in insiemi per il training e per il test, come visto sopra, e per ogni iterazione si genera un modello che viene valutato tramite delle metriche.
\begin{figure}
    \centering
...
\begin{algorithmic}[1]
\STATE $S_1, S_2, ..., S_k$ = partizionamento($S$, $k$)
...
\end{algorithmic}
\end{algorithm}
Per poter effettivamente implementare la grid search si fa quella che viene definita \textit{cross validation annidata}: si opera una cross validation esterna per prevenire l'overfitting e poi, per ciascuna fold, si fa una cross validation interna che ha l'obiettivo di testare tutte le combinazioni indicate nella griglia e selezionare i modelli che hanno ottenuto i risultati migliori.
\\
La procedura descritta in precedenza viene, pertanto, estesa, come mostrato nell'Algoritmo \ref{nested}.
\begin{algorithm}
...
\STATE test = $S_i$
\STATE train = $S \setminus S_i$
\STATE $S_1, S_2, ..., S_L$ = partizionamento(train, $l$)
