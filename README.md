...
# Model Selection
Assuming you have preprocessed training and test sets in ```preprocessed_datasets``` folder, you can run ```main_model_selection.py``` in order to perform a tuning phase such that the optimal configuration can be obtained.
Once the configuration is found, a model is created and is directly stored in ```selecte_models/models.csv``` file.

# Experiments
If you want to launch the final experiments to see results, you have to run ```main_experiment``` which select a model from ```selected_models/models.csv``` and run an experiment on the test set.
